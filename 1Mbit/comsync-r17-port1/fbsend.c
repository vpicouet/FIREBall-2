...
// some FIREBALL definitions
#define FB_DEFAULT_NONBLOCK 1
#define FB_DEFAULT_SHOW_EVENTS 1
#define FB_DEFAULT_USE_SELECT 1
#define FB_DEFAULT_PACKET_SIZE 256
#define FB_DEFAULT_DELAY 1
#define FB_MAX_PACKET_SIZE 256
#define FB_MIN_PACKET_SIZE 16
#define FB_MAX_DELAY 100
#define FB_MIN_DELAY 1
...
void GetDataOpParams(struct _port_args *Params, int mode)
{
  ...
  // we are using globals here, because we can. 
  // removing the queries.
  Params->packetSize = input_packet_size;
  
  // removed the Rx and RxTx delay questions and settings
  Params->delayTx = input_delayTx * 1000;
...
void *thread_write(void *arg) {
	...
	// this is a threaded application, but it does not need to be. 
	// we are only making this so because we are adapting a prior test program.
...
int main(int argc, char **argv) {
	...
	// defaults
	packet_size=FB_DEFAULT_PACKET_SIZE;
	delay=FB_DEFAULT_DELAY; 

	fflag=0;
	opterr=0;
	while ((c = getopt (argc, argv, "p:d:f:h")) != -1)
	  switch (c)
	    {
	    case 'p':
	      // packet size option
	      cvalue = optarg;
	      packet_size=atoi(cvalue);
	      if (packet_size > FB_MAX_PACKET_SIZE){
		packet_size=FB_MAX_PACKET_SIZE;
	      } else if (packet_size < FB_MIN_PACKET_SIZE) {
		packet_size=FB_MIN_PACKET_SIZE;
	      };
	      break;
	    case 'd':
	      ...
	      cvalue = optarg;
	      delay = atoi(cvalue);
	      if ( delay > FB_MAX_DELAY ){ 
		delay = FB_MAX_DELAY;
	      } else if (delay < FB_MIN_DELAY) { 
		delay = FB_MIN_DELAY;
	      };
	      break;
	    case 'f':
		...
		// file doesn't exist
		printf("File '%s' is not readable. Exiting.\n",filename);
		fprintf(stderr,"File '%s' is not readable. Exiting.\n",filename);
		return -1;
	      }
	      // check if the file exists
	      break;
	    case 'h':
	      printf("Usage:\n\t%s -f filename <-d delay> <-p packet_size>\n",argv[0]);
	      printf("\t\tdelay default = 1; packet_size default = 256\n");
	      return 0;
	      break;
	    case '?':
	...
	// transfer the loaded in values to globals that are used by the transmit thread.
	input_packet_size = packet_size;
	input_delayTx = delay;
	...
	// we are hard coding these options.
	// 1. Non-block = YES
	port_args1.non_block_io = FB_DEFAULT_NONBLOCK;


	// 2. Print events? YES
	port_args1.show_events = FB_DEFAULT_SHOW_EVENTS;

	// 3. Use select API? YES
	port_args1.use_select = FB_DEFAULT_USE_SELECT;


	// not touching any of the other stuff here. 

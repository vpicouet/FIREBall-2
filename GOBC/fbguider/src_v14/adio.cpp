...
namespace adio{
  ...
  void retrieve_pt_multi(float *temp,float *pressure,float *gobc_temp, float *gobc_pressure,
    ...
    //average_pt(&tnew, &pnew, temp, pressure);
    fblog::logmsg("PT: %.3f %.3f %.3f",ai_input[0],ai_input[1],ai_input[2]);
    // ICA 08/08/2023: adding extra readouts for GOBC container, Calibration box.
    static float gobc_tnew, gobc_pnew;
    *gobc_tnew = ai_input[3]*1000.0-273.15; // 
    // convert from kPa to mbar
    *gobc_pnew = (250.*(ai_input[4]/ai_input[0])+10)*10;
    //average_pt(&gobc_tnew, &gobc_pnew, gobc_temp, gobc_pressure);
    fblog::logmsg("GOBC PT: %.3f %.3f\n",*gobc_temp, *gobc_pressure);
    static float cal_tnew, cal_pnew;
    *cal_tnew = ai_input[5]*1000.0-273.15; // 
    // convert from kPa to mbar
    *cal_pnew = (250.*(ai_input[6]/ai_input[0])+10)*10;
    //average_pt(&cal_tnew, &cal_pnew, cal_temp, cal_pressure);
    fblog::logmsg("CAL BOX PT: %.3f %.3f",ai_input[5],ai_input[6]);
    average_pt_multi(&tnew, &pnew, temp, pressure,
      &gobc_tnew, &gobc_pnew, gobc_temp, gobc_pressure,
      &cal_tnew, &cal_pnew, cal_temp, cal_pressure);
  ...
  void average_pt_multi(float *newt, float *newp, float *t, float *p,
    ...
    static float gobc_temps[PTBUFSIZE],gobc_pressures[PTBUFSIZE];
    static float cal_temps[PTBUFSIZE],cal_pressures[PTBUFSIZE];
    ...
    gobc_temps[idx]=*gobc_newt;
    gobc_pressures[idx]=*gobc_newp;

    cal_temps[idx]=*cal_newt;
    gobc_pressures[idx]=*gobc_newp;
    ...
    ttmp=0;ptmp=0;
    gobc_ttmp=0;gobc_ptmp=0;
    cal_ttmp=0;cal_ptmp=0;

    for(i=0;i<maxctr;i++){
      ttmp+=temps[i];
      ptmp+=pressures[i];
      gobc_ttmp+=gobc_temps[i];
      gobc_ptmp+=gobc_pressures[i];
      cal_ttmp+=cal_temps[i];
      cal_ptmp+=cal_pressures[i];

    };
    
    *t=ttmp/maxctr;
    *p=ptmp/maxctr;
    *gobc_t=gobc_ttmp/maxctr;
    *gobc_p=gobc_ptmp/maxctr;
    *cal_t=cal_ttmp/maxctr;
    *cal_p=cal_ptmp/maxctr;

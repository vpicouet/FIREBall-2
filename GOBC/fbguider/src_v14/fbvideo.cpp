...
//				       video_width, video_height,
	...
	return -1;
      ...
      // create the renderer
      // MM 220618 -- something is causing the TARGETTEXTURE below to not work.
      // suspect it is some misbehavior, misinstall of OpenGL.
	  ...
	  if(frameinfo -> use[idx]){
	    if(fbm == MultiStarFollow){
	     //DRAW_CROSS(frameinfo->tx[idx]-frameinfo->x[idx]+box_center_x[idx],
			 ...
			 //1);
               DRAW_DIAMOND(frameinfo->tx[idx]-frameinfo->x[idx]+box_center_x[idx],
       frameinfo->ty[idx]-frameinfo->y[idx]+box_center_y[idx], 1.5, 1);
	    } else {

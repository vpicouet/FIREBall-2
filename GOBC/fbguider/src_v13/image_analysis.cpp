...
namespace img{
  ...
  void *imageworker(void *data){
	...
	if (valid[idx]) {
      ...
      compile_guide_stats(dx,dy,drot);
  ...
  void get_guide_stats(float *dx, float *dy, float *dr, float *rmsdx, float *rmsdy, float *rmsdr){
    *dx=offval[0];
    *rmsdx=offrms[0];
    *dy=offval[1];
    *rmsdy=offrms[1];
  ...
  void compile_guide_stats(float dx, float dy, float dr){
    static char initialized=0;
    static float mx[OFFMAX], my[OFFMAX], mr[OFFMAX], xsq[OFFMAX], ysq[OFFMAX], rsq[OFFMAX];
    static unsigned char index;
    static float xsum,ysum,rsum,xsqsum,ysqsum,rsqsum;
    if ( ! initialized ) {
      for(int i=0; i< OFFMAX; i++)
	mx[i]=my[i]=mr[i]=xsq[i]=ysq[i]=rsq[i]=0.0;
      index=0;
      initialized=1;
    }; // not initialized...

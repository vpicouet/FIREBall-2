...
# -------------------------------------------------------------------
# adding a pipe for guider acquire readouts - ICA 08/10/2023 
# -------------------------------------------------------------------
#set pipe_path "/home/salo/FB/Fireball2/fbground_v14/acquire_pipe"
#if {![file exists $pipe_path]} {
#    if {[catch {exec mkfifo $pipe_path}]} {
#        puts "Error creating named pipe"
#        exit 1
#        }
#    }
#set acq_pipe [open "acquire_pipe" a]
#puts $logfile [starttime]
...
proc writepipe {msg} {
    ...
    #puts $pipe_path
    #puts $msg
    set acq_pipe [open "./acquire_pipe" "w"]
    puts $acq_pipe $msg

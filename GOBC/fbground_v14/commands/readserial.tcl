...
#	puts " "
	...
	switch -- $mid {
		...
		for { set i 0 } { $i < 8 } { incr i } {
		    ...
		    #puts $f
		    if {$expo > 0 && $f > 0} {
		    	set mag [ expr {-2.5 * log10( $f / $expo) + 13.4 } ]
		    } else {
		    	set mag 0.0
		    }
...
#		set var::ai_pressure $aip
	    ...
	    240 {
		binary scan $data ffffffff dx dy dr rx ry rr msx msy
		...
		#set var::msx [ format {%0.4f} $msx ]
		#set var::msy [ format {%0.4f} $msy ]
	    writepipe "RCVD $ct: 240 $var::dx $var::dy $var::dr $var::rmsdx $var::rmsdy $var::rmsdr $var::msx $var::msy"
	    }
...
#	puts $vals
	...
	puts "charblah incoming"
	scan $blah %c charblah
	scan $endb %c charendb
	if {[info exists charblah]} {
		puts "charblah: $charblah $charendb"		
	} else {
		puts "this is where charblah crashes"	
	}

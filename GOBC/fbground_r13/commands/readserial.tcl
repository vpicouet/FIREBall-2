...
#		set var::lampBstatus [ expr ( $diobyte & 0x40 )>0 ]
	    ...
	    247 { 
		binary scan $data ccccisuisssffs gm dm alg sub fn tag et ccdt camt pst ait aip savep
		set str "RCVD $ct: 247 $gm $dm $alg $sub $fn $tag $et $ccdt $camt $pst $ait $aip $savep"
		writelog $str
...
#		set var::ai_pressure $aip
	...
	### Added mm 180914
	### If there is junk on the line, the two bytes representing lenght will be random
	### those still get parsed and ingested in as the $length variable. 
	### 'length' is technically a short int, but it gets read in as a short. 
	### later comparisons rely on it being positive (the crash reported by Nicole 
	### happened when the puts "message..." line below gave a length of -6911.

	### so we make it a signed short

	set length [ expr { $length & 0xFFFF} ]

	### now, the problem will be that when there is noise on the line, this length could be **anything**
	### so I am adding a print line below that will give you the current buffer length.
	...
	### Added mm 180914
	if { [string length $buffer] > [expr 2000]} {
	    puts "Warning: buffer length may be excessive (gt 2000): $lb"
	}
	### -------
